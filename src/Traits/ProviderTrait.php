<?php
namespace Omaicode\Laradules\Traits;

use Omaicode\Laradules\Supports\Helper;
...
trait ProviderTrait
{
    ...
    protected function getConfigFilePath($file): string
    {
        ...
        if (!file_exists($file) && Str::contains($file, base_path('/modules'))) {
    ...
    protected function throwInvalidPluginError()
    {
        ...
        $from = str_replace('/Providers', '', dirname($reflection->getFilename()));
        ...
        if ($from != $to) {
            throw new Exception(sprintf('Module folder is invalid. Need to rename folder %s to %s', $from, $to));
        }
    }
